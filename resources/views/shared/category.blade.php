...
@section('title', 'Category: ' . $share->category->name)

@section('content')
  <div style="margin-bottom: 24px; display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 16px;">
      <div>
          <span style="font-size: 0.9rem; font-weight: 600; color: var(--accent-color); text-transform: uppercase; letter-spacing: 1px;">Shared Folder / Category</span>
          <h1 style="font-family: var(--font-accent); font-size: 2.2rem; margin: 4px 0 0 0; color: var(--text-main);">
              <i class="fa-solid fa-folder-open" style="color: var(--accent-color); margin-right: 8px;"></i> {{ $share->category->name }}
          </h1>
          <div style="margin-top: 8px; color: var(--text-muted); font-size: 0.9rem;">
              Shared by <strong style="color: var(--text-main);">{{ $share->user->name }}</strong>
          </div>
      </div>
          ...
          <div id="active-tasks-list" style="display: flex; flex-direction: column; gap: 16px;">
              @forelse ($tasks as $task)
                  @php
                      ...
                      $taskDetailUrl = route('shared.category.task', ['categoryToken' => $token, 'taskId' => $task->id]);
                  @endphp
...
(function() {
    ...
    function buildActiveCard(task) {
        ...
        const detailUrl = '/shared/category/' + channelToken + '/task/' + task.id;
    ...
    function updateCounters() {
        ...
        if (activeEl && activeList) activeEl.textContent = activeList.querySelectorAll('.task:not(.completed)').length;
        if (completedEl && completedSection) completedEl.textContent = completedSection.querySelectorAll('.task.completed').length;
    }
    ...
    window.Echo.channel('shared.category.' + channelToken)
        ...
        .listen('.task.updated', function(e) {
            ...
            if (task.status === 'completed') {
                ...
                if (list) { list.insertAdjacentHTML('afterbegin', buildCompletedCard(task)); completedSection.style.display = ''; }
            } else {
                if (existing) {
                    existing.outerHTML = buildActiveCard(task);
                } else {
                    const completedCard = document.getElementById('task-card-' + task.id);
